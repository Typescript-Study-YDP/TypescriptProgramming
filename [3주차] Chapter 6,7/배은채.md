...
공변(Covariance) : A가 B의 서브타입이면, T<A>는 T<B>의 서브타입이다.<br/>
반변(Contravariance) : A가 B의 서브타입이면, T<B>는 T<A>의 서브타입이다.<br/>
양변(Bivariance) : A가 B의 서브타입이면, T<A> → T<B>도 되고 T<B> → T<A>도 되는 경우<br/>
불변(immutability) : A가 B의 서브타입이더라도, T<A> → T<B>도 안 되고 T<B> → T<A>도 안 되는 경우<br/>

타입스크립트에서의 "타입"들은 기본적으로 "공변"성 규칙을 따르지만,<br/> 
유일하게 "함수의 매개변수"는 "반변성"을 갖고 있다.<br/>
(tsconfig의 strictFunctionTypes 옵션이 true 일 때 기준) <br/>

- 공변: 같거나 서브타입인 경우, 할당 가능 <br/>
